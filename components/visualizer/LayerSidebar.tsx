    ...
    <aside className="scroll-thin z-40 flex h-full w-72 shrink-0 flex-col overflow-y-auto border-r border-outline-variant bg-surface-container-low/95 backdrop-blur-xl md:bg-surface-container-low/80">
      <div className="flex flex-1 flex-col gap-md p-md">
        <div className="flex items-center gap-2 border-b border-outline-variant pb-md">
          ...
          <h2 className="font-label-caps text-label-caps text-primary">Encapsulation</h2>
        </div>
        ...
        <div>
          <label className="mb-1.5 block font-label-caps text-[10px] text-on-surface-variant">HEADERS</label>
          <div className="flex flex-col gap-1.5">
            {LEGEND.map((l) => (
              <div key={l.label} className="border border-outline-variant px-2 py-1.5">
                <span className={`inline-block border px-1.5 py-px font-mono text-[10px] font-bold ${l.tone}`}>
                  {l.label}
                </span>
                <p className="mt-1 font-body-sm text-[10px] leading-snug text-on-surface-variant/65">{l.what}</p>
              </div>
            ))}
          </div>
        </div>

        <div className="border-l-2 border-coral/60 bg-coral/5 px-2.5 py-2">
          <p className="font-label-caps text-[9px] tracking-wider text-coral">THE COST</p>
