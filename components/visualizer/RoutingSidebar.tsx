    ...
    <aside className="scroll-thin z-40 flex h-full w-72 shrink-0 flex-col overflow-y-auto border-r-[1.5px] border-dashed border-outline-variant bg-surface-container-low/95 backdrop-blur-xl md:bg-surface-container-low/80">
      <div className="flex flex-1 flex-col gap-md p-md">
        <div className="flex items-center gap-2 border-b-[1.5px] border-dashed border-outline-variant pb-md">
          ...
          <h2 className="font-hand text-[17px] font-bold text-primary">{isForwarding ? "Forwarding" : "Routing algorithms"}</h2>
        </div>
        <SidebarTabs />
        ...
        {isForwarding ? (
          ...
          <Field label="Link costs" hint="Bellman-Ford recomputes every route from these costs.">
            <div className="flex flex-col gap-1.5">{links.map((link) => <div key={link.id} className="grid grid-cols-[1fr_72px] items-center gap-2"><span className="font-mono text-[11px] text-on-surface-variant">{link.id.replace("r-", "").replaceAll("-", " ↔ ")}</span><NumberInput value={link.cost} onCommit={(cost) => run({ linkCosts: { ...params.linkCosts, [link.id]: cost } })} min={1} max={15} /></div>)}</div>
          </Field>
        )}
        <div className="flex flex-col gap-2 border-t-[1.5px] border-dashed border-outline-variant pt-md">
          <label className="flex items-center gap-1.5 font-label-caps text-[9px] uppercase tracking-[0.08em] text-coral"><Icon name="warning" className="text-[13px]" /> Faults</label>
        ...
        <button onClick={() => run()} className="mt-auto flex w-full items-center justify-center gap-2 rounded-md border-[1.5px] border-primary py-2.5 font-hand text-[15px] font-bold text-primary transition-colors hover:bg-primary hover:text-surface active:scale-[0.98]"><Icon name="play_circle" className="text-[18px]" /> Re-run</button>
      </div>
    </aside>
  );
}
