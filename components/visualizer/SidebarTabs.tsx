    ...
    <div>
      ...
      <div className="grid gap-1" style={{ gridTemplateColumns: `repeat(${columns}, minmax(0,1fr))` }}>
        {siblings.map((l) => {
          ...
          const cls = `flex flex-col items-center gap-0.5 rounded-md border-[1.5px] px-1 py-1.5 transition-colors ${
            selected
              ? "border-primary bg-primary/12 text-primary"
              : disabled
                ? "cursor-not-allowed border-dashed border-outline-variant/50 text-on-surface-variant/35"
                : "border-dashed border-outline-variant text-on-surface-variant hover:border-primary/70 hover:text-on-surface"
          }`;
          const body = (
            <>
              <Icon name={l.icon} className="text-[16px]" />
              <span className="w-full truncate text-center font-label-caps text-[8px] leading-tight">
                {l.title}
              </span>
            </>
          );
